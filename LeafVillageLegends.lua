...
function LeafVE:ResetAllLeafPoints()
  EnsureDB()
  -- Clear all leaf point totals
  LeafVE_DB.alltime = {}
  LeafVE_DB.lboard.alltime = {}
  LeafVE_DB.lboard.weekly = {}
  LeafVE_DB.lboard.updatedAt = {}
  -- Clear per-day global point history so AggForThisWeek returns zeros
  LeafVE_DB.global = {}
  -- Update the persisted leaderboard cache to reflect the reset
  if LeafVE_GlobalDB.leaderboardCache then
    LeafVE_GlobalDB.leaderboardCache = { achievementPoints = {}, leafPoints = {}, weekly = {}, lifetime = {}, lastUpdate = Now() }
  end
  ...
  -- Broadcast a forced resync so guild members refresh their caches
  if InGuild() then
    SendAddonMessage("LeafVE", "LEAFPTSRESETALL", "GUILD")
    -- Give peers a moment to process, then broadcast our (now empty) data
    local broadcastTimer = 0
    local bFrame = CreateFrame("Frame")
    bFrame:SetScript("OnUpdate", function()
      broadcastTimer = broadcastTimer + arg1
      if broadcastTimer >= 2 then
        LeafVE:BroadcastLeaderboardData()
        LeafVE:SendResyncRequest()
        bFrame:SetScript("OnUpdate", nil)
      end
    end)
  end
...
function LeafVE:MergeShoutoutHistory(payload)
  ...
  -- Persist latest state to cache so it survives logout
  if updated then
    LeafVE:UpdateLeaderboardCache()
  end
end
...
function LeafVE:OnAddonMessage(prefix, message, channel, sender)
  ...
  if message == "LEAFPTSRESETALL" then
    -- Another admin has reset all Leaf Points; clear our local copies too
    EnsureDB()
    LeafVE_DB.alltime = {}
    LeafVE_DB.lboard.alltime = {}
    LeafVE_DB.lboard.weekly = {}
    ...
    Print("|cFFFF4444A guild admin has reset all Leaf Points.|r")
    return
  end
...
-- Snapshot the current leaderboard state into GlobalDB so it survives logout/reload.
-- achievementPoints: { [name] = totalAchievementPts, ... }  (from achievementCache)
-- leafPoints / lifetime: { [name] = { L, G, S, total }, ... }
-- weekly: { [name] = { L, G, S, total }, ... }  (current week)
function LeafVE:UpdateLeaderboardCache()
  EnsureDB()
  if not LeafVE_GlobalDB.leaderboardCache then
    LeafVE_GlobalDB.leaderboardCache = { achievementPoints = {}, leafPoints = {}, weekly = {}, lifetime = {}, lastUpdate = 0 }
  end
  local cache = LeafVE_GlobalDB.leaderboardCache
  cache.lastUpdate = Now()

  -- Lifetime / leaf-points snapshot
  cache.lifetime = {}
  cache.leafPoints = {}
  local allKnown = {}
  for name, _ in pairs(LeafVE_DB.alltime or {}) do allKnown[name] = true end
  for name, _ in pairs(LeafVE_DB.lboard.alltime or {}) do allKnown[name] = true end
  for name, _ in pairs(allKnown) do
    local lp = LeafVE_DB.alltime[name]
    local sp = LeafVE_DB.lboard.alltime[name]
    local best
    if lp and sp then
      local lt = (lp.L or 0) + (lp.G or 0) + (lp.S or 0)
      local st = (sp.L or 0) + (sp.G or 0) + (sp.S or 0)
      best = lt >= st and lp or sp
    else
      best = lp or sp or {L = 0, G = 0, S = 0}
    end
    local total = (best.L or 0) + (best.G or 0) + (best.S or 0)
    cache.lifetime[name]   = { L = best.L or 0, G = best.G or 0, S = best.S or 0, total = total }
    cache.leafPoints[name] = total
  end
  ...
  local weekKnown  = {}
  for name, _ in pairs(localWeek) do weekKnown[name] = true end
  for name, _ in pairs(syncedWeek) do weekKnown[name] = true end
  for name, _ in pairs(weekKnown) do
    local lp = localWeek[name]
    local sp = syncedWeek[name]
    local best
    if lp and sp then
      local lt = (lp.L or 0) + (lp.G or 0) + (lp.S or 0)
      local st = (sp.L or 0) + (sp.G or 0) + (sp.S or 0)
    ...
    cache.weekly[name] = { L = best.L or 0, G = best.G or 0, S = best.S or 0, total = (best.L or 0) + (best.G or 0) + (best.S or 0) }
  end

  -- Achievement points snapshot
  cache.achievementPoints = {}
  if LeafVE_GlobalDB.achievementCache then
    for name, data in pairs(LeafVE_GlobalDB.achievementCache) do
      if type(data) == "table" and data.totalPoints then
        cache.achievementPoints[name] = data.totalPoints
      end
    end
  end
end
...
-- Confirmation popup for "Reset All Leaf Points"
StaticPopupDialogs["LEAFVE_CONFIRM_RESET_LEAF_POINTS"] = {
  text = "|cFFFF4444Reset ALL Leaf Points?|r\n\nThis will zero out every player's current, weekly, and lifetime Leaf Point totals and cannot be undone.",
  button1 = "Confirm Reset",
  button2 = "Cancel",
  OnAccept = function()
    LeafVE:ResetAllLeafPoints()
  end,
  timeout = 0,
  whileDead = true,
  hideOnEscape = true,
...
local function BuildAdminPanel(panel)
  ...
  -- Section: Leaf Points Reset
  local resetLPSection = subFrame:CreateFontString(nil, "OVERLAY", "GameFontNormal")
  resetLPSection:SetPoint("TOPLEFT", subFrame, "TOPLEFT", 12, yBase)
  resetLPSection:SetText("|cFF2DD35CLeaf Points Reset|r")
  yBase = yBase - 28

  local resetLPHint = subFrame:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
  resetLPHint:SetPoint("TOPLEFT", subFrame, "TOPLEFT", 12, yBase)
  resetLPHint:SetText("|cFF888888Zeros out all players' current, weekly, and lifetime Leaf Point totals.|r")
  resetLPHint:SetWidth(430)
  resetLPHint:SetJustifyH("LEFT")
  yBase = yBase - 22

  local resetAllLeafPtsBtn = CreateFrame("Button", nil, subFrame, "UIPanelButtonTemplate")
  resetAllLeafPtsBtn:SetWidth(180)
  resetAllLeafPtsBtn:SetHeight(22)
  resetAllLeafPtsBtn:SetPoint("TOPLEFT", subFrame, "TOPLEFT", 12, yBase)
  resetAllLeafPtsBtn:SetText("Reset All Leaf Points")
  SkinButtonAccent(resetAllLeafPtsBtn)
  resetAllLeafPtsBtn:SetScript("OnClick", function()
    StaticPopup_Show("LEAFVE_CONFIRM_RESET_LEAF_POINTS")
  end)
  yBase = yBase - gap
...
ef:SetScript("OnEvent", function()
  ...
  if event == "PLAYER_LOGIN" then
    ...
    -- Seed leaderboard state from the persisted cache so the UI shows
    -- last-known values immediately before any guild sync arrives.
    EnsureDB()
    if LeafVE_GlobalDB.leaderboardCache then
      local cache = LeafVE_GlobalDB.leaderboardCache
      -- Populate lboard.alltime from cached lifetime data if local data is missing
      if cache.lifetime then
        for name, entry in pairs(cache.lifetime) do
          if not LeafVE_DB.lboard.alltime[name] then
            LeafVE_DB.lboard.alltime[name] = { L = entry.L or 0, G = entry.G or 0, S = entry.S or 0 }
          end
        end
      end
      -- Populate lboard.weekly (current week) from cached weekly data if missing
      if cache.weekly then
        local wk = WeekKey()
        if type(LeafVE_DB.lboard.weekly[wk]) ~= "table" then
          LeafVE_DB.lboard.weekly[wk] = {}
        end
        for name, entry in pairs(cache.weekly) do
          if not LeafVE_DB.lboard.weekly[wk][name] then
            LeafVE_DB.lboard.weekly[wk][name] = { L = entry.L or 0, G = entry.G or 0, S = entry.S or 0 }
          end
        end
      end
    end
    ...
    broadcastFrame:SetScript("OnUpdate", function()
      ...
      if broadcastTimer >= 5 then
        if InGuild() then
          ...
          LeafVE:BroadcastLeaderboardData()
          -- Also request fresh data from all online guildies
          LeafVE:SendResyncRequest()
...
updateFrame:SetScript("OnUpdate", function()
  ...
  -- Sync badges and refresh leaderboards every 5 minutes
  if badgeSyncTimer >= 300 then
    badgeSyncTimer = 0
    if InGuild() then
      LeafVE:BroadcastBadges()
      LeafVE:BroadcastLeaderboardData()
      -- Request fresh leaderboard data from all online guildies
      LeafVE:SendResyncRequest()
    end
    -- Persist latest leaderboard state to cache (works even without a guild)
    LeafVE:UpdateLeaderboardCache()
  end
